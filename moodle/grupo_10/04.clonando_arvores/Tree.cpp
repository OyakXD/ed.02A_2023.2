...
bool Tree::_identical(Node *nd1, Node *nd2) {
    // Se ambos forem nullptr, são iguais.
    if (nd1 == nullptr && nd2 == nullptr) {
        return true;
    }

    // Se apenas um for nullptr, são diferentes.
    if (nd1 == nullptr || nd2 == nullptr) {
        return false;
    }

    // Se a key for diferente, são diferentes.
    if (nd1->key != nd2->key) {
        return false;
    }

    // Se chegou aqui, a key é igual e ambos são não-nulos.
    // Verifica recursivamente se os filhos são iguais.
