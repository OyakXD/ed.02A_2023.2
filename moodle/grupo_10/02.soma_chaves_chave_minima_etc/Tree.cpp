...
int Tree::_sum_keys(Node *node) { 
    ...
    // Soma da chave do no atual com a soma das chaves das subarvores
...
int Tree::_min_key(Node *node) { 
    ...
    if (node->left != nullptr) {
        min_left = _min_key(node->left);
    }

    if (node->right != nullptr) {
        min_right = _min_key(node->right);
    }

    return std::min(node->key, std::min(min_left, min_right));
...
int Tree::_um_filho(Node *node) {
    ...
    return count + _um_filho(node->left) + _um_filho(node->right);
