...
class Matriz {
   private:
    ...
    int** mat;  // ponteiro para matriz de inteiros (deve ser alocada
                // dinamicamente)
   ...
   public:
    ...
    // Recebe uma referencia para o objeto B do tipo Matriz que possui
    // m linhas e n colunas e multiplica a matriz que chamar essa funcao-membro
    // pela matriz B. Lembre que a multiplicacao de uma matriz A por uma matriz
    // B so eh possivel se o numero de colunas de A for igual ao numero de
    // linhas de B. Se for possivel fazer a multiplicacao, um ponteiro para uma
    // nova matriz C contendo o resultado da multiplicacao deve ser retornado
    // (note que a matriz C deve ser alocada dinamicamente dentro da
    // funcao-membro). Caso contrario, retorna nullptr indicando que nao foi
    // possivel fazer a multiplicacao das matrizes.
    ...
    // instanciar uma matriz passando outra matriz como argumento pelo
    // construtor
